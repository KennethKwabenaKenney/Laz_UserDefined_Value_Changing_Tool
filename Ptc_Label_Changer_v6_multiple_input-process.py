...
class PointCloudLabelChanger:
    def __init__(self, root):
        ...
        switch_method_button = tk.Button(self.label_frame, text="Switch Method", command=self.switch_label_entry_method)
    ...
    def update_label_entry_method(self):
        ...
        if method == "Manual Entry":
            ...
            messagebox.showinfo("Excel Format", "Please ensure that the Excel sheet has *old labels* in the first column and *new labels* in the second column with their associated headings.")
    ...
    def load_excel_sheet(self):
        ...
        if file_path:
            ...
            # Optionally, you can read the Excel file here and display the labels in the GUI
            try:
                ...
                changes_str = "\n".join([f"Change {i+1}: Old Label {old} to New Label {new}"
                                        for i, (old, new) in enumerate(zip(old_labels, new_labels))])
                self.label_changes_var.set(changes_str)
    
                # Append the new label changes to the existing list
                self.label_changes.extend(list(zip(old_labels, new_labels)))
    ...
    def add_label_change(self):
        ...
        if method == "Manual Entry":
            ...
            try:
                ...
                # Update the label_changes list
                self.label_changes = list(zip(old_labels, new_labels))
            except Exception as e:
                messagebox.showerror("Error", f"Error reading Excel file: {e}")
                return
    ...
    def change_label(self):
        if self.point_cloud is not None:
            try:
                # Check if label changes are provided from Excel
                if self.label_changes:
                    #Also Checking the Processing Method
                    ...
                    # Iterate through the list of label changes from Excel
                    ...
                    # Iterate through the list of manually entered label changes
                    for i, (old_label, new_label) in enumerate(self.label_changes, start=1):
                        # Convert labels to int
                        try:
                            old_label = int(old_label)
...
#%% Function for Single / Batch Process
    ...
    def process_batch_method(self):
            ...
            # Find all files with the same extension in the directory
            file_pattern = os.path.join(directory, '*' + os.path.splitext(self.file_path)[1])
            files = glob.glob(file_pattern)
    
            for file_path in files:
                # Read each point cloud file
                point_cloud = self.read_point_cloud(file_path)
    
                # Apply label changes to each point cloud
                self.apply_label_changes(point_cloud)
    
                # Save each point cloud individually
                self.save_updated_point_cloud(file_path, point_cloud)
                
    def apply_label_changes(self, point_cloud):
            # Iterate through the list of label changes
            for old_label, new_label in self.label_changes:
                # Convert labels to int
                try:
                ...
                # Modify the Ext_Class field
                mask = np.isclose(point_cloud[:, -1], old_label)
                point_cloud[mask, -1] = new_label

    def save_updated_point_cloud(self, original_file_path, point_cloud):
        # Save the updated point cloud with new labels
        file_path, _ = os.path.splitext(original_file_path)
        updated_file_path = f"{file_path}_updated.laz"
            
        # Writing out the new point cloud
        out_las = laspy.create(file_version="1.4", point_format=7)
        out_las.header.offset = [point_cloud[:, 0].min(), point_cloud[:, 1].min(), point_cloud[:, 2].min()]
        out_las.header.scale = [0.1, 0.1, 0.1]
        out_las.x = point_cloud[:, 0]
        out_las.y = point_cloud[:, 1]
        out_las.z = point_cloud[:, 2]
        out_las.intensity = point_cloud[:, 3]
        out_las.red = point_cloud[:, 12] if point_cloud.shape[1] > 12 else 0
        out_las.green = point_cloud[:, 13] if point_cloud.shape[1] > 13 else 0
        out_las.blue = point_cloud[:, 14] if point_cloud.shape[1] > 14 else 0
        out_las.gps_time = point_cloud[:, 6] if point_cloud.shape[1] > 6 else 0
        out_las.classification = point_cloud[:, -1]
    
        # Ensure that 'Ext_Class' attribute is available in 'point_cloud'
        if 'Ext_Class' in out_las.point_format.dimension_names:
            ext_class_index = list(out_las.point_format.dimension_names).index('Ext_Class')
            if ext_class_index < point_cloud.shape[1]:
                out_las.Ext_Class = point_cloud[:, ext_class_index]
    
        # Save the updated point cloud
        out_las.write(updated_file_path)
#%% Writting out the laz/las file
    # def write_las(self, file_path, point_cloud):
    #     # Read the original LAS file
    #     original_ptcloud_path = self.file_path  
    #     original_ptcloud = laspy.read(original_ptcloud_path)
